...
# Load dataset
```{r}
if(!file.exists("/workdir/dwm269/scCardiacOrganoid/robjs/")){
  system("Rscript scripts/r/convert_h5ad_to_seurat.R")
}else(
  load("/workdir/dwm269/scCardiacOrganoid/robjs/scCO_v7-1.RData")
)
...
DimPlot(
  scco.seu,
  group.by="leiden_harmony_sp_anno",
...
scco.chat <- createCellChat(
  ...
  meta = data.frame(
    leiden_harmony_types = as.factor(scco.seu$cell_types),
    row.names = Cells(scco.seu)
  ),
...
scco.chat@DB <- CellChatDB.human

# subset the expression data of signaling genes for saving computation cost
scco.chat <- subsetData(
  scco.chat
  # features = Features(scco.seu)[!Features(scco.seu) %in% c("H2-Q8", "H2-T9", "H2-T18", "H2-Q9" ,"H2-L", "H2-BI" ,"H2-D" ,"H60a" ,"H2-Ea-ps")]
  )

# CellChat processing (memory intensive for large datasets)
scco.chat <- identifyOverExpressedGenes(scco.chat)
scco.chat <- identifyOverExpressedInteractions(scco.chat)
gc()
```
...
scco.chat <- projectData(scco.chat, PPI.human)

# scco.chat@idents = droplevels(scco.chat@idents, exclude = setdiff(levels(meta$labels),unique(meta$labels)))


scco.chat <- computeCommunProb(scco.chat)

# Filter out the cell-cell communication if there are only few number of cells in certain cell groups
scco.chat <- filterCommunication(scco.chat, min.cells = 10)

scco.chat <- computeCommunProbPathway(scco.chat)

scco.chat <- aggregateNet(scco.chat)

# Compute the network centrality scores
scco.chat <- netAnalysis_computeCentrality(
  scco.chat, 
  slot.name = "netP"
) # the slot 'netP' means the inferred intercellular communication network of signaling pathways

save(scco.chat,file="/workdir/dwm269/scCardiacOrganoid/robjs/scCO_cellchat_v1.RData")
